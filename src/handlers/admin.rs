...
use crate::{service::training::TrainingService, types::{MyDialogue, MyResult}};
...
pub async fn callback_show_time_admin(
    ...
    if let Some(m) = message {
        if let Some(d) = data {
            ...
            let trainings = training_serivce.get_trainings_by_date_with_registration(day).await;
            let mut my_message = format!("<b> Записи на {}:</b>\n\n", day.format("%d.%m"));
            for t in trainings {
                let l = format!("{} — {}, @{} \n", t.start_time.format("%H:%M"), t.full_name, t.username);
                my_message += &l;
            };
            bot.edit_message_text(m.chat().id, m.id(), my_message).parse_mode(teloxide::types::ParseMode::Html).await?;
            dialogue.update(State::Default).await?;
        } else {
            bot.edit_message_text(m.chat().id, m.id(), "Извините, что-то пошло не так")
                .await?;
